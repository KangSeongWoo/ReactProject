...
const DepositOne = props => {
    ...
    const [state, setState] = useState({
        rowData: null,
        columnDefs: [
            { field: 'depositKey', headerName: '기타입고번호', editable: false, suppressMenu: true },
    ...
    const setView = async () => {
        try {
            let res = await Https.getDepositOneEtc(getData)
    ...
    return (
        <>
            <CustomBreadcrumb style={{ marginBottom: '0px' }} arr={['기타입출고', '기타입고내역']}></CustomBreadcrumb>
