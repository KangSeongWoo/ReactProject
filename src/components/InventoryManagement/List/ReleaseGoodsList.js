...
const ReleaseGoodsList = props => {
    ...
    const columnDefs = () => {
        return [
            ...
            { field: 'depositKey', headerName: '츌고번호', editable: false, suppressMenu: true },
            {
                field: 'storageId',
                headerName: '출고창고',
                editable: false,
            ...
            {
                field: 'depositGb', headerName: '기타출고구분', editable: false, suppressMenu: true,
                valueFormatter: function(params) {
            ...
            { field: 'rackNo', headerName: '출고렉', editable: false, suppressMenu: true }
        ]
    }
    ...
    const setInit = async () => {
        ...
        try {
            ...
            setStorageList(res.data.data.Storages) // 출고창고 State
   ...
   // 조회
    const checkTheValue = async () => {
        ...
        return await Https.getReleaseListEtc(params, config)
            .then(response => {
                props.setSpin(false)
    ...
    const goDetail = e => {
        ...
        window
            .open(
                '/#/InventoryManagement/releaseOne?' + queryStirng.stringify(e.data),
                '상세' + new Date(),
    ...
    return (
        <>
            <CustomBreadcrumb style={{ marginBottom: '0px' }} arr={['기타입출고', '기타출고리스트']}></CustomBreadcrumb>
...
export default withRouter(ReleaseGoodsList)
